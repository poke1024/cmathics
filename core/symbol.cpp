...
void Messages::add(
    ...
    const unicode_t* unicode_text = reinterpret_cast<const unicode_t*>(text);

    m_rules.add(
		std::make_shared<RewriteRule>(
			expression(definitions.symbols().MessageName, name, Heap::String(unicode_tag)),
			Heap::String(unicode_text)));
}
